...
class BTree_Ex_Generator:
    ...
    def __init__(
        ...
        # create insert tree
        if bbaum == None:
            self.bbaum = BTree_Creator(height, numbersUntil, M, treeType).myBBaum
        else:
            self.bbaum = bbaum

        self.operationsAndSol = []  # array of the class BTree_OneOp
        self.howManyOperations = self.checkOperationCount(
            operationCount, treeType == TreeTypes.INSERT
        )
    ...
    def generateOperationValues(self, values):
        if self.howManyOperations > 0:
            if values != None:
                ...
                leafNodes = [
                    node
                    for node in self.bbaum.nodeArray
                    if node.nextNodes.count(None) == len(node.nextNodes)
                ]
    ...
    def createOperationValues(self, leafNodes):
        ...
        if str(self.treeType) == str(TreeTypes.INSERT) or str(self.treeType) == str(
            TreeTypes.INSERT_TRIVIAL
        ):
            if str(self.treeType) == str(TreeTypes.INSERT):
                ...
                fullLeaf = next(
                    (
                        x
                        for x in leafNodes
                        if len(x.values) == self.bbaum.valuesCountInNode
                    ),
                    None,
                )
                if fullLeaf != None:
                    value = int(
                        np.random.random_integers(
                            low=fullLeaf.values[0], high=fullLeaf.values[-1]
                        )
                    )
                ...
                if notFullLeaf != None:
                    value = int(
                        np.random.random_integers(
                            low=notFullLeaf.values[0], high=notFullLeaf.values[-1]
                        )
                    )
                    tmpOperationValues.append(value)
                    self.howManyOperations -= 1
                    ...
                    # no full leafNode in Tree
                    warnings.warn(
                        "Was not able to ask for insertion in a not full leaf."
                    )
            # other can be random
            minVal = allValuesOfTheTree[0]
            maxVal = allValuesOfTheTree[-1]
            while self.howManyOperations > 0:
                value = int(np.random.random_integers(low=minVal, high=maxVal))
                tmpOperationValues.append(value)
                self.howManyOperations -= 1

        elif (
            str(self.treeType) == str(TreeTypes.DELETE_VERSCHMELZ)
            ...
            if not str(self.treeType) == str(TreeTypes.DELETE_TRIVIAL):
                (ausgleichLeaf, verschmelzLeaf) = self.findAusgleichVerschmelzLeaf(
                    leafNodes
                )
                ...
                for i, node in enumerate([ausgleichLeaf, verschmelzLeaf]):
                    if node == None:
                        if i == 0:
                            if str(self.treeType) == TreeTypes.DELETE_AUSGLEICH:
                                warnings.warn(
                                    "Was not able to ensure ausgleich operation"
                                )
                                break
                        else:
                            if str(self.treeType) == TreeTypes.DELETE_VERSCHMELZ:
                                warnings.warn(
                                    "Was not able to ensure verschmelz operation"
                                )
                                break
                        continue
                    ...
                    if node != None and self.howManyOperations > 0:
                        index = int(
                            np.random.random_integers(low=0, high=len(node.values) - 1)
                        )
                        # this loop is important
                        stopInfinityLoop = 10
                        while (
                            not node.values[index] in allValuesOfTheTree
                            and stopInfinityLoop > 0
                        ):
                            index = int(
                                np.random.random_integers(
                                    low=0, high=len(node.values) - 1
                                )
                            )
                            stopInfinityLoop -= 1
                ...
                # is trivial
                notHalfFullLeaf = next(
                    (x for x in leafNodes if len(x.values) > self.bbaum.halffull), None
                )
            ...
            while len(allValuesOfTheTree) > 0 and self.howManyOperations > 0:
                ...
                if len(allValuesOfTheTree) > 1:
                    index = int(
                        np.random.random_integers(
                            low=0, high=len(allValuesOfTheTree) - 1
                        )
                    )
    ...
    def findAusgleichVerschmelzLeaf(self, leafNodes):
        ...
        for i, node in enumerate(leafNodes):
            ...
            if len(node.values) == self.bbaum.halffull:
                ...
                if ausgleichLeaf == None and (
                    leftMoreThanHalffull or rightMoreThanHalffull
                ):
                    ausgleichLeaf = node
